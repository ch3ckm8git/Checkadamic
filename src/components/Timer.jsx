...
export default function Timer({ onFinish }) {
    ...
    const stop = () => {
        ...
        // ✅ Correct mapping: preserve sub-mode, else map main special cases
        let backendMode;
        if (mode === "sub") {
            backendMode = "sub";
        } else {
        ...
        const session = {
            id: Date.now(),
            mode: backendMode, // "sub" for subtasks; backend will skip counting
            mainTask: mode === "main" ? mainTask : "",
            subTask: mode === "sub" ? subTask : "",
            title: title || mainTask || subTask,
            desc,
            seconds: goalMinutes * 60 - seconds, // elapsed
            goalMinutes,
            startTime,
            endTime: end,
            pausedTime: totalPaused,
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <button
                            onClick={() => {
                                if (mode === "main") {
                                    setMode("sub");
                                    setMainTask(""); // clear main when switching to sub
                                } else {
                                    setMode("main");
                                    setSubTask(""); // clear sub when switching to main
                                }
                                setTitle("");
                                setDesc("");
                            }}
